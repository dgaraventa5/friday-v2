...
export function EditReminderModal({ reminder, open, onOpenChange, onSave }: EditReminderModalProps) {
  ...
  useEffect(() => {
    if (reminder) {
      ...
      setTimeLabel(reminder.time_label ? reminder.time_label.substring(0, 5) : '');
      setRecurrenceType(reminder.recurrence_type);
      setRecurrenceInterval(reminder.recurrence_interval.toString());
      
      if (reminder.recurrence_type === 'weekly') {
        setSelectedDays(reminder.recurrence_days || []);
      } else if (reminder.recurrence_type === 'monthly') {
        setMonthlyType(reminder.monthly_type || 'day_of_month');
        if (reminder.monthly_type === 'day_of_month') {
          setMonthlyDayOfMonth((reminder.recurrence_days?.[0] || 1).toString());
        } else {
          setMonthlyWeekday((reminder.recurrence_days?.[0] || 1).toString());
          setMonthlyWeekPosition((reminder.monthly_week_position || 1).toString());
        }
      }
      
      setEndType(reminder.end_type);
      setEndCount((reminder.end_count || 10).toString());
      setError('');
    }
  }, [reminder]);
      ...
      <DialogContent className="dialog-sheet max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-title">Reminder Name</Label>
            <Input
              id="edit-title"
              value={title}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="edit-never" className="cursor-pointer font-normal">
                  Never
                </Label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="cursor-pointer"
                />
                <Label htmlFor="edit-after" className="cursor-pointer font-normal">
                  After
                </Label>
                {endType === 'after' && (
                  <>
